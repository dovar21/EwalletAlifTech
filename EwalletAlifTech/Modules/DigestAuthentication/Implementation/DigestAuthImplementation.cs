...
namespace EwalletAlifTech.Modules.DigestAuthentication.Implementation
{
    internal class DigestAuthImplementation
    {
            ...
            IClock clock)
        {
            ...
            _usernameHashedSecretProvider = usernameHashedSecretProvider ?? throw new ArgumentNullException(nameof(usernameHashedSecretProvider));
        ...
        public async Task<string> BuildAuthInfoHeaderAsync(DigestChallengeResponse response) {
            ...
            List<ValueTuple<string, string, bool>> parts = new() {
                ("qop", QopMode, true),
        ...
        private static string GenerateExpectedHash(DigestChallengeResponse response, string requestMethod, string a1Hash) {
        ...
        private static string CreateRspAuth(DigestChallengeResponse response, string a1Hash)
        {
            var a2 = $":{response.Uri}";
            var a2Hash = a2.ToMD5Hash();
